# Job board — foundation checklist
...
- [x] Foundation exit test — isolated Postgres proof recorded `skipped_disabled`, two distinct Greenhouse boards, a one-row-per-board dedupe result, a duplicate audit count, and a per-board rate-limit rejection; no external ATS request was made.
- [ ] Step 1 — not started.
