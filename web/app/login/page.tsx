...
export default function LoginPage() {
  ...
  const isWebFlow = !isExtensionFlow;
  ...
  useEffect(() => {
    ...
    console.log('  - redirect:', dashboardUrl);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      if (isExtensionFlow) {
        ...
        const callbackUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/auth/callback?next=${encodeURIComponent(dashboardUrl)}`;
    ...
    try {
      ...
      if (isExtensionFlow) {
        ...
        if (!sessionData.ok) {
          ...
          window.location.href = '/auth/extension?redirect=' + encodeURIComponent(dashboardUrl);
        } else {
