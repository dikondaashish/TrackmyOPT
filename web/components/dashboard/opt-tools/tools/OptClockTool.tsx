...
export function OptClockTool() {
  ...
  const loadSavedData = async () => {
    try {
      const [optRes, spansRes, profileRes] = await Promise.all([
      ...
      if (optRes.ok) {
        ...
        if (data.status) {
          if (data.status.opt_start_date) setOptStartDate(formatDateForInput(data.status.opt_start_date));
          if (data.status.opt_ead_end_date) setOptEndDate(formatDateForInput(data.status.opt_ead_end_date));
        }
      }
