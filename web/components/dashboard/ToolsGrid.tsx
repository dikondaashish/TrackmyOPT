...
export function ToolsGrid() {
  ...
  const checkExtensionInstalled = useCallback(async (): Promise<boolean> => {
    ...
    if (typeof chrome === "undefined" || !chrome.runtime) {
      console.log("🔍 Not in Chrome or chrome.runtime unavailable");
      return false;
    }

    if (!EXTENSION_ID) {
      console.log("⚠️ Extension ID not configured");
      return false;
    }
        ...
        chrome.runtime!.sendMessage(
          ...
          (response) => {
            if (chrome.runtime?.lastError) {
              console.log("❌ Extension not installed:", chrome.runtime.lastError.message);
              resolve(false);
