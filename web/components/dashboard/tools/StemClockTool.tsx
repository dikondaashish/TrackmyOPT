      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((day, i) => (
          <button key={i} disabled={!day} onClick={() => day && onSelect(formatDateInput(new Date(currentMonth.getFullYear(), currentMonth.getMonth(), day)))}
            className={`p-2 text-sm rounded-lg ${!day ? 'invisible' : selected?.getDate() === day && selected?.getMonth() === currentMonth.getMonth() ? 'bg-purple-500 text-white' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
            {day}
          </button>
        ))}
      </div>
...
export function StemClockTool() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.ok && data.data) {
            if (data.data.stem_start_date) setStemStartDate(data.data.stem_start_date);
          }
        }
      } catch {}
    };
  ...
  const calculate = () => {
    ...
    const stemDays = Math.max(0, totalDays - employedDays);
    const priorDays = parseInt(priorUnemployment) || 0;
    const used = stemDays + priorDays;
    const max = 150;
    setResults({ used, remaining: Math.max(0, max - used), max, priorDays, stemDays });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <DateInput label="STEM OPT Start Date" value={stemStartDate} onChange={setStemStartDate} description="From your STEM EAD card" />
        <DateInput label="STEM OPT End Date" value={stemEndDate} onChange={setStemEndDate} description="From your STEM EAD card" />
      </div>

      {/* Prior Unemployment */}
      <div className="p-4 rounded-xl border bg-gray-50 dark:bg-gray-800/50">
        <label className="font-semibold text-gray-900 dark:text-white block mb-2">Prior OPT Unemployment Days</label>
        <p className="text-sm text-gray-500 mb-3">Days accumulated during your initial OPT period (0-90)</p>
        <Input type="number" value={priorUnemployment} onChange={(e) => setPriorUnemployment(e.target.value)} placeholder="0" min="0" max="90" />
      </div>
      ...
      <div className="space-y-3">
        ...
        {employmentSpans.length === 0 ? (
          <p className="text-sm text-gray-500 text-center py-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
            No jobs added yet. Add your STEM employment periods.
          </p>
        ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-white dark:bg-gray-800 border text-center">
              <p className="text-sm text-gray-500 mb-1">Prior OPT</p>
              <p className="text-2xl font-bold">{results.priorDays}</p>
            </div>
            <div className="p-4 rounded-xl bg-white dark:bg-gray-800 border text-center">
              <p className="text-sm text-gray-500 mb-1">STEM</p>
              <p className="text-2xl font-bold">{results.stemDays}</p>
            </div>
            <div className="p-4 rounded-xl bg-white dark:bg-gray-800 border text-center">
              <p className="text-sm text-gray-500 mb-1">Remaining</p>
              <p className="text-2xl font-bold text-green-600">{results.remaining}</p>
            </div>
          </div>
        </div>
      )}
