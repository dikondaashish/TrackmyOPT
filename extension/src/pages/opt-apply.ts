...
export function renderOptApply(root: HTMLElement, onBack: () => void): void {
  ...
  calculateBtn.addEventListener('click', async () => {
    ...
    // Render the new countdown-style results
    renderCountdownResults(resultsContainer, results);
  });
...
/**
 * Render countdown-style results
 */
async function renderCountdownResults(
  ...
  function updateCountdown() {
    ...
    const daysLeftText = container.querySelector('#days-left-text');
    ...
    const messageEl = container.querySelector('#time-message');
  ...
  // Event listeners
  const modifyBtn = container.querySelector('#modify-dates-btn');
  if (modifyBtn) {
  ...
  if (isPremium) {
    ...
    if (saveEmailBtn) {
      saveEmailBtn.addEventListener('click', () => {
        const emailInput = container.querySelector('#reminder-email-input') as HTMLInputElement;
    ...
    const stopBtn = container.querySelector('#stop-reminders-btn');
    if (stopBtn) {
    ...
    const upgradeBtn = container.querySelector('#upgrade-premium-btn');
    if (upgradeBtn) {
