export const OPT_DATE_FIELDS = [
  ...
  'opt_ead_end_date', 'stem_start_date', 'stem_dso_recommendation_date',
] as const;
...
export function calculatorDatePatch(body: unknown) {
  ...
  const patch: Partial<Record<typeof OPT_DATE_FIELDS[number], string | null>> = {};
  for (const field of OPT_DATE_FIELDS) {
    if (Object.prototype.hasOwnProperty.call(input, field)) patch[field] = parseCalculatorDate(input[field]);
  }
  const fields = Object.keys(patch);
  if (!fields.length) throw new Error('At least one date field is required');
  const lastUpdatedField = typeof input._lastModifiedField === 'string' && fields.includes(input._lastModifiedField)
    ? input._lastModifiedField : fields[fields.length - 1];
  return { ...patch, last_updated_field: lastUpdatedField };
}

export function formatCalculatorDate(iso: string | null | undefined): string | null {
  if (!iso) return null;
