...
export interface AiGenerationLimitState {
  allowed: boolean;
  quotaPeriod: 'day' | 'month'; quotaLimit: number; quotaRemaining: number;
  dailyLimit: number; dailyRemaining: number;
  itemRegenerationLimit: number; itemRegenerationsRemaining: number;
  resetsAt?: string; error?: 'ai_daily_limit_reached'|'ai_monthly_limit_reached'|'ai_item_regeneration_limit_reached'|'ai_rate_limited';
}
export interface ScreeningQuestionDraftResponse extends Omit<AiGenerationLimitState, 'error'> {
  ...
  error?: 'sensitive'|'insufficient_context'|'limit'|'generation_failed'|'feature_disabled'|'not_implemented'|'ai_daily_limit_reached'|'ai_monthly_limit_reached'|'ai_item_regeneration_limit_reached'|'ai_rate_limited';
}
