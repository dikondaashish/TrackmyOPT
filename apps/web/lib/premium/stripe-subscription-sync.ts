...
function getStripePriceIdMap(): Record<PlanId, { month?: string; year?: string }> {
  return {
    pro: {
...
function getSubscriptionPeriodEndIso(subscription: Stripe.Subscription): string {
...
function extractHostedInvoiceUrl(subscription: Stripe.Subscription): string | null {
  const inv = subscription.latest_invoice;
  if (!inv || typeof inv !== "object") return null;
