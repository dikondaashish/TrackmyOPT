...
// The browser receives ApplyBolt's response directly, as in the manual tool.
...
export function BrowserEmailRecovery({ contact, targetRole, userIntent }: {
  ...
  return <div className="mt-3 space-y-3" aria-label={`Email recovery for ${contact.name}`}>
    ...
    {result && !email && <p role="status" className="text-sm text-slate-700 dark:text-slate-200">{result.status === 'not_found' ? 'ApplyBolt did not find a work email for this profile.' : result.status === 'provider_error' ? 'ApplyBolt returned an unexpected response. Please try again.' : 'ApplyBolt could not verify a work email at this company.'}</p>}
    {email && <>
      <p role="status" className="break-all text-sm text-slate-800 dark:text-slate-100"><a href={`mailto:${email}`} className="font-medium text-blue-700 underline dark:text-blue-300">{email}</a> · Verified by ApplyBolt in this browser</p>
