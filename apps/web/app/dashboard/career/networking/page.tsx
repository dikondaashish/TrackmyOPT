...
export default async function NetworkingPage({
  ...
  const applications = (data ?? []) as ApplicationOption[];

  return (
    <main className="mx-auto max-w-6xl space-y-7 px-3 py-5 sm:px-6 sm:py-8">
      <Link
        href="/dashboard/career"
        className="inline-flex min-h-11 items-center gap-2 rounded-lg text-sm font-medium text-slate-600 hover:text-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:text-slate-300 dark:hover:text-blue-300"
      >
        <ArrowLeft className="size-4" aria-hidden="true" /> Back to Career Hub
      </Link>
      ...
      <header className="flex flex-wrap items-start gap-4">
        ...
        <div className="max-w-2xl space-y-2">
          ...
          <p className="text-base leading-7 text-slate-600 dark:text-slate-300">
            Connect your application to a relevant person, find a work email
            from their LinkedIn profile, and prepare a message you can review
            before sending.
          </p>
        </div>
      </header>

      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.7fr)_minmax(260px,0.85fr)] lg:items-start">
        <section className="min-w-0 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-7">
          <NetworkingWorkspace
            ...
            applicationsUnavailable={Boolean(error)}
          />
        </section>
        ...
        <aside className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-900/50 sm:p-6">
          ...
          <ol className="space-y-4 text-sm leading-6 text-slate-600 dark:text-slate-300">
            <li>
              <span className="mr-2 font-semibold text-blue-700 dark:text-blue-300">
                01
              </span>
              Choose an application or add company and role context.
            </li>
            <li>
              <span className="mr-2 font-semibold text-blue-700 dark:text-blue-300">
                02
              </span>
              Find a relevant person on LinkedIn and paste their profile.
            </li>
            <li>
              <span className="mr-2 font-semibold text-blue-700 dark:text-blue-300">
                03
              </span>
              Review the email result and edit your AI drafts before contacting
              them.
            </li>
          </ol>
          <p className="border-t border-slate-200 pt-4 text-xs leading-5 text-slate-600 dark:border-slate-700 dark:text-slate-300">
            You provide contact profiles. Email verification does not guarantee
            delivery. Gmail and Outlook open a prefilled draft for you to review
            and send.
          </p>
          <Link
