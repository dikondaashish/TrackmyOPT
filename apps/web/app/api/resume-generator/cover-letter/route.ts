...
export async function POST(req:NextRequest) {
  ...
  const {snapshot,sourceContentHash,job,isRegeneration}=parsed.data;
  const limit=await consumeAiGeneration(userId,`${job.companyName}|${job.roleTitle}|${sourceContentHash}`,isRegeneration === true);
