...
import { calculatorDatePatch, formatCalculatorDate as formatDate } from '@/lib/immigration/opt-date-patch';
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      .from('opt_status')
      .select('program_end_date, dso_recommendation_date, stem_dso_recommendation_date, opt_start_date, opt_ead_end_date, stem_start_date, last_updated_field')
      .eq('user_id', userId)
      .single();
    ...
    return NextResponse.json({
      ...
      data: data ? {
        ...
        stem_dso_recommendation_date: formatDate(data.stem_dso_recommendation_date),
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { ok: false, error: 'Provide at least one valid date (MM/DD/YYYY), or null to clear it' },
