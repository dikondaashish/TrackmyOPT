...
function request(spans: unknown) {
  return new NextRequest('https://www.trackmyopt.com/api/employment-spans', {
    method: 'POST',
...
function query(error: unknown = null) {
  const q = {
    ...
    select: vi.fn(),
    single: vi.fn(async () => ({
      data: error ? null : { id: 'saved' },
      error,
    })),
  };
  for (const key of ['insert', 'update', 'eq', 'select'] as const)
...
it.each([undefined, null, [], {}].map((value) => [value]))(
  'rejects missing/empty spans: %j',
  async (spans) => {
    expect((await POST(request(spans))).status).toBe(400);
  }
);

it.each([undefined, 'existing-job'])(
  'reports database failure for insert/update: %s',
  async (id) => {
...
it('saves same-day employment and leap days without shifting dates', async () => {
  ...
  expect(
    (
      await POST(
        request([
          { ...valid, start_date: '02/29/2024', end_date: '02/29/2024' },
        ])
      )
    ).status
  ).toBe(200);
...
it('scopes updates to the signed-in user and allows current jobs', async () => {
  ...
  mocks.from.mockReturnValue(q);
  expect(
    (await POST(request([{ ...valid, id: 'existing-job', end_date: null }])))
      .status
  ).toBe(200);
...
it.each([undefined, 'existing-job'])(
  'saves a normalized user-confirmed website on insert/update: %s',
  async (id) => {
    const q = query();
    mocks.from.mockReturnValue(q);
    expect(
          ...
          request([
            {
              ...valid,
              id,
              employer_domain: 'https://www.example.com/about?secret=no',
            },
          ])
        )
      ).status
    ).toBe(200);
    expect(id ? q.update : q.insert).toHaveBeenCalledWith(
      expect.objectContaining({ employer_domain: 'example.com' })
    );
  }
);

it('allows explicitly clearing a website', async () => {
  const q = query();
  mocks.from.mockReturnValue(q);
  expect(
    (
      await POST(
        request([{ ...valid, id: 'existing-job', employer_domain: null }])
      )
    ).status
  ).toBe(200);
  expect(q.update).toHaveBeenCalledWith(
    expect.objectContaining({ employer_domain: null })
  );
});
...
it.each([
  ...
  'rejects bad websites before any batch writes: %s',
  async (employer_domain) => {
    const q = query();
    mocks.from.mockReturnValue(q);
    expect(
      (await POST(request([valid, { ...valid, employer_domain }]))).status
    ).toBe(400);
    expect(q.insert).not.toHaveBeenCalled();
    expect(q.update).not.toHaveBeenCalled();
  }
