...
export function JobCardActions({ jobId, companyName, title, jobUrl, sponsorId }: JobCardActionsProps) {
  ...
  return (
    <section className="border-t border-slate-950/10 bg-white/25 px-5 py-4 dark:border-white/10 dark:bg-slate-950/25 sm:px-6" aria-label="Safe job actions">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {jobUrl && (
          <a
            ...
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-slate-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200"
          >
            View role <ExternalLink className="size-4" aria-hidden="true" />
          </a>
        )}
        <details className="group relative">
          ...
          <div className="absolute left-0 z-10 mt-2 grid w-[min(21rem,calc(100vw-3rem))] gap-1 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-950">
            <Link
      ...
      {showFollowup && (
        <div className="flex flex-wrap items-end gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900" aria-label="Set a follow-up date">
          <label className="grid gap-1 text-sm font-medium text-slate-800 dark:text-slate-100">
            ...
            <input
              ...
              className="min-h-11 rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
            />
          </label>
          <button
            type="button"
      ...
      <p className="text-xs leading-5 text-slate-500 dark:text-slate-400">These actions only save information you choose to your tracker or open existing tools. They never submit an application or contact an employer.</p>
    </section>
  );
}
