...
function normalizeCompanies(data: unknown): Company[] {
  ...
  return data
    .filter((item): item is { name: string; domain: string; brandId?: string } =>
      !!item && typeof item.name === 'string' && !!item.name.trim() && typeof item.domain === 'string'
    )
    .slice(0, MAX_SUGGESTIONS)
    .map((item) => ({
      id: typeof item.brandId === 'string' && item.brandId ? item.brandId : `${item.name}\u0000${item.domain}`,
      name: item.name,
      domain: item.domain,
    }));
}

function statusMessage(visible: boolean, result: SearchState | null, count: number): string {
  if (!visible) return '';
  if (result?.status === 'loading') return 'Searching companies…';
  if (result?.status === 'error') return 'Suggestions unavailable. You can still enter your employer name.';
  return count ? `${count} suggestions. Use arrow keys and Enter to select.` : 'No matches. You can keep the name you typed.';
}

export function useEmployerNameSuggestions(value: string, onChange: (value: string) => void) {
  ...
  const select = useCallback((company: Company) => {
    onChange(company.name);
    dismiss();
  }, [dismiss, onChange]);

  const visible = open && result?.query === value.trim();
  const companies = useMemo(() => visible && result ? result.companies : [], [result, visible]);
  ...
  return {
    ...
    onBlur: dismiss,
    onChange: (text: string) => { onChange(text); search(text); },
    onCompositionEnd: (text: string) => { composing.current = false; search(text); },
