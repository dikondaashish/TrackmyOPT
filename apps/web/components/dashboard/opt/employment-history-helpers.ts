import { calendarDateISO, employmentWindowStats, estimatedStemEndISO, localTodayISO } from '@/lib/immigration/calendar-days';
import { addDays, calculateUnemploymentDays, formatDate, isoToMMDDYYYY } from '@/lib/immigration/opt-calculations';
...
export const EMPTY_EMPLOYMENT_STATS: EmploymentStats = {
  totalEmployedDays: 0, totalUnemployedDays: 0, currentStreak: 0, longestGap: 0,
};
export { formatDate as formatEmploymentDate };

export function calculateEmploymentDuration(start: string, end: string | null): string {
...
export function toEmploymentInputDate(value: string | null | undefined): string {
...
export function computeEmploymentStats(
  spans: EmploymentSpan[], optStartDate: string | undefined, optEndDate: string | undefined,
  stemStartDate?: string, asOfISO = localTodayISO(),
): EmploymentStats {
  ...
  const extension = activeStem ? employmentWindowStats(activeStem, estimatedStemEndISO(activeStem), spans, asOfISO) : null;
  const continuous = activeStem && Date.parse(activeStem) === Date.parse(initialEnd) + 86400000
    ? employmentWindowStats(start, estimatedStemEndISO(activeStem), spans, asOfISO) : null;
  ...
  const compliance = calculateUnemploymentDays(start, end, spans, stem, null, asOfISO);
  return {
    totalEmployedDays: initial.totalEmployedDays + (extension?.totalEmployedDays || 0),
    totalUnemployedDays: compliance.used,
    longestGap: continuous?.longestGap ?? Math.max(initial.longestGap, extension?.longestGap || 0),
