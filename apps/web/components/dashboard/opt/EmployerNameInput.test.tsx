...
const companies = [{ name: 'Amazon', domain: 'amazon.com' }, { name: 'Amazon Web Services', domain: 'aws.amazon.com' }];
const response = (data: unknown = companies) => ({ ok: true, json: async () => data });
function Form() { const [value, setValue] = useState(''); return <EmployerNameInput value={value} onChange={setValue}/>; }
const type = (value: string) => fireEvent.change(screen.getByRole('combobox'), { target: { value } });
const tick = async (ms = 300) => act(async () => { await vi.advanceTimersByTimeAsync(ms); });
...
it('waits for three characters and debounces requests using only the public client ID', async () => {
  render(<Form/>); type('Am'); await tick(); expect(fetch).not.toHaveBeenCalled();
  type('Ama'); await tick(150); type('Amaz'); await tick(299); expect(fetch).not.toHaveBeenCalled(); await tick(1);
...
it('supports keyboard selection without changing anything until Enter', async () => {
  render(<Form/>); type('Ama'); await tick();
...
it('selects with a click and allows editing the legal employer name', async () => {
  ...
  type('Amazon Web Services, Inc.'); expect(screen.getByRole('combobox')).toHaveValue('Amazon Web Services, Inc.');
});
...
it.each([[], null, [{ name: null, domain: 'bad.test' }]].map(data => [data]))('allows manual names with no usable results: %j', async data => {
  vi.mocked(fetch).mockResolvedValue(response(data) as Response);
  render(<Form/>); type('My company'); await tick();
...
it('keeps manual entry available after a rate-limit failure', async () => {
  ...
  render(<Form/>); type('My employer'); await tick();
...
it('ignores stale responses and aborts previous requests', async () => {
  ...
  type('Mic'); expect(signal.aborted).toBe(true);
  vi.mocked(fetch).mockResolvedValue(response([{ name: 'Microsoft', domain: 'microsoft.com' }]) as Response); await tick();
  await act(async () => { resolveOld(response() as Response); });
...
it('cancels pending searches on blur, short input and unmount', async () => {
  const view = render(<Form/>); type('Ama'); fireEvent.blur(screen.getByRole('combobox')); await tick();
  expect(fetch).not.toHaveBeenCalled(); type('Amaz'); type('A'); await tick(); expect(fetch).not.toHaveBeenCalled();
  type('Amazon'); view.unmount(); await tick(); expect(fetch).not.toHaveBeenCalled();
});

it('shows at most five suggestions and renders response text safely', async () => {
  vi.mocked(fetch).mockResolvedValue(response(Array.from({ length: 8 }, (_, i) => ({ name: '<img onerror=bad>' + i, domain: `company${i}.com` }))) as Response);
  render(<Form/>); type('Com'); await tick();
  expect(screen.getAllByRole('option')).toHaveLength(5);
  expect(document.querySelector('img')).toBeNull();
});

it('does not search while composing text', async () => {
  render(<Form/>); fireEvent.compositionStart(screen.getByRole('combobox')); type('Ama'); await tick(); expect(fetch).not.toHaveBeenCalled();
  fireEvent.compositionEnd(screen.getByRole('combobox')); await tick(); expect(fetch).toHaveBeenCalledTimes(1);
});
...
it('times out a stalled lookup without blocking manual entry', async () => {
  ...
  render(<Form/>); type('Example'); await tick();
...
it('selection in the real employment form changes only employer and never saves automatically', async () => {
  const employer = vi.fn(), start = vi.fn(), end = vi.fn(), current = vi.fn(), submit = vi.fn();
  render(<EmploymentSpanForm employer="" startDate="09/01/2026" endDate="09/22/2026" isCurrent={false} saving={false}
    submitLabel="Save Employment" onEmployerChange={employer} onStartDateChange={start} onEndDateChange={end}
    onIsCurrentChange={current} onCancel={vi.fn()} onSubmit={submit}/>);
  // Controlled parent receives typed input; render its updated value.
  type('Ama');
  cleanup();
  render(<EmploymentSpanForm employer="Ama" startDate="09/01/2026" endDate="09/22/2026" isCurrent={false} saving={false}
    submitLabel="Save Employment" onEmployerChange={employer} onStartDateChange={start} onEndDateChange={end}
    onIsCurrentChange={current} onCancel={vi.fn()} onSubmit={submit}/>);
  fireEvent.keyDown(screen.getByRole('combobox'), { key: 'ArrowDown' }); await tick();
  fireEvent.click(screen.getByRole('option', { name: /^Amazon\s*amazon.com$/ }));
  expect(employer).toHaveBeenLastCalledWith('Amazon');
  for (const callback of [start, end, current, submit]) expect(callback).not.toHaveBeenCalled();
