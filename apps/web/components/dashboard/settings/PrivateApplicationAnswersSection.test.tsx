...
describe('PrivateApplicationAnswersSection secret lifecycle', () => {
  ...
  it('explains Prefill before revealing saved values, without a per-application approval gate', () => {
    const fetchMock = vi.fn();
    vi.stubGlobal('fetch', fetchMock);
    render(<PrivateApplicationAnswersSection />);
    expect(screen.getByText(/No separate approval step/)).toHaveTextContent(
      'Prefill this application'
    );
    expect(screen.getByText(/Saved portal login/)).toHaveTextContent(
      'password confirmation'
    );
    expect(
      screen.queryByText(/must approve them for each application/)
  ...
  it('does not fetch/decrypt until Review is clicked and clears plaintext on Hide', async () => {
    ...
    expect(fetchMock).not.toHaveBeenCalled();
    fireEvent.click(
      screen.getByRole('button', { name: /edit login & answers/i })
    );

    await screen.findByRole('button', { name: /hide saved details/i });
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(screen.getAllByDisplayValue('Review-only!9A')).toHaveLength(2);

    fireEvent.click(
      screen.getByRole('button', { name: /hide saved details/i })
    );
  ...
  it('shows one shared login and discloses cross-portal reuse', async () => {
    ...
    expect(
      screen.getByText(/^Prefill uses this same saved login/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/if one portal is compromised/i)
    ).toBeInTheDocument();
  });
  ...
  it('requires an explicit choice before a legacy login becomes the default', async () => {
    ...
    expect(
      screen.queryByDisplayValue('Legacy-only!9A')
    ).not.toBeInTheDocument();
    ...
    fireEvent.click(
      screen.getByRole('button', { name: /save login & answers/i })
    );

    await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(2));
    const saveInit = fetchMock.mock.calls[1]?.[1] as RequestInit;
    expect(
      await screen.findByText(/Private application data saved/)
    ).toHaveTextContent('Prefill this application');
