...
export function renderStemClockTracker(
  ...
  // STEM unemployment is cumulative with initial OPT. A STEM start date alone
  // cannot produce a truthful deadline; verified usage is loaded below.
  ...
  countdownCard.innerHTML = `
    <div id="verified-clock-status" style="text-align:center;">
      <div style="font-size:14px;font-weight:750;">Loading verified cumulative unemployment usage…</div>
      <div style="font-size:11px;opacity:.8;margin-top:8px;">Includes initial OPT and STEM OPT employment records</div>
    </div>
  `;
  ...
  void loadVerifiedUnemploymentClock()
    .then((clock) => {
      if (!clock || clock.phase !== 'stem') {
        countdownCard.innerHTML = `
          <div style="font-size:14px;font-weight:750;text-align:center;">A verified STEM unemployment clock is not available yet.</div>
          <div style="font-size:11px;line-height:1.5;opacity:.85;text-align:center;margin-top:8px;">Add your initial OPT dates, STEM start date, and complete employment history in the dashboard. A STEM start date by itself cannot calculate remaining days.</div>
        `;
        return;
      }
      ...
      countdownCard.innerHTML = `
        ...
        <div style="height:10px;background:var(--surface-2);border-radius:999px;overflow:hidden;margin-top:16px;">
          <div id="verified-stem-clock-progress" style="height:100%;width:0;background:linear-gradient(90deg,#10b981,#f59e0b,#dc2626);border-radius:999px;transition:width .65s ease;"></div>
        </div>
        <div style="font-size:11px;line-height:1.45;opacity:.8;text-align:center;margin-top:12px;">The 150-day maximum is cumulative across initial OPT and STEM OPT. Update employment records whenever work changes. This is a tracking aid, not legal advice.</div>
      `;
    ...
    .catch(() => {
      countdownCard.innerHTML = `
        <div style="font-size:13px;font-weight:700;text-align:center;">Could not load verified cumulative unemployment usage.</div>
        <div style="font-size:11px;opacity:.8;text-align:center;margin-top:8px;">Open the dashboard and review all OPT/STEM employment records.</div>
      `;
    });
