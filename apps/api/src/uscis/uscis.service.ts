...
export class UscisService {
    ...
    async checkUSCISStatus(receiptNumber: string): Promise<USCISStatus> {
        ...
        const baseUrl = this.configService.get('USCIS_API_BASE_URL') || 'https://api-int.uscis.gov/case-status';
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`USCIS API Error ${response.status}: ${response.statusText}`);
            }
    ...
    private async getAccessToken(): Promise<string | null> {
        ...
        const tokenUrl = this.configService.get('USCIS_TOKEN_URL') || 'https://api-int.uscis.gov/oauth/accesstoken';
