...
export class UscisProcessor {
    ...
    async handleCheckStatus(job: Bull.Job<{ receiptNumber: string, userId: string }>) {
        const { receiptNumber, userId } = job.data;
        this.logger.log(`Checking status for ${receiptNumber} (User: ${userId})...`);
        ...
        try {
            ...
            if (error) {
                this.logger.error(`Failed to update DB for ${receiptNumber}: ${error.message}`);
